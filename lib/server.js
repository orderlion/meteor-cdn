...
function CdnController() {
  ...
  function validateSettings(rootUrl, cdnUrl) {
    ...
    if (cdn.host === root.host) {
      console.warn(
        "CDN: CDN HOST === ROOT HOST. Using default Meteor behaviour"
      );
      return false;
    }
  ...
  function static404connectHandler(req, res, next) {
    ...
    console.log("static404connectHandler cookie", req.headers.cookie);
    // console.log('static404connectHandler cookies', req.cookies);
    // console.log('static404connectHandler static files', WebAppInternals.staticFilesByArch[clientArch]);
